...
export default function MedicalAttachmentsScreen() {
  const user = useUserStore((s) => s.user);
  const viewerId = user?.id ?? '';
  const viewerRole = 'medical' as const;
  ...
  return (
    <View style={styles.container}>
      <Stack.Screen options={{ title: 'Patient Attachments' }} />

      <Card style={styles.infoCard}>
        <View style={styles.infoRow}>
          <Users2 size={18} color={colors.accent.primary} />
          <Text style={styles.infoText}>Select a patient to view files shared with medical professionals.</Text>
          <TouchableOpacity
            onPress={() => talkService.speak('Select a patient to view attachments that have been shared with medical professionals.')}
            style={styles.speakBtn}
            testID="speak-medical-attachments-info"
      ...
      <FlatList
        ...
        ListEmptyComponent={() => (
          <Text style={styles.emptyText} testID="patients-empty">No assigned patients.</Text>
        )}
      />
